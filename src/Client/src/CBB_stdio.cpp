...
extern "C" FILE *CBB_WRAP(fopen)(const char* path, const char* mode)
{
	...
	if(CBB_client::_interpret_path(formatted_path.c_str()))
	{
		...
		return client.open_stream(relative_path.c_str(), mode);
	}
	else
	{
		MAP_BACK(fopen);
...
extern "C" FILE *CBB_WRAP(freopen)(const char* path, const char* mode, FILE* stream)
{
	...
	if(nullptr != stream && client.interpret_stream(stream))
	{
		client.close_stream(stream);
		_LOG("CBB freopen path=%s, mode=%s\n", path, mode);
		return client.open_stream(path, mode);
	}
	else
	{
		MAP_BACK(freopen);
...
extern "C" int CBB_WRAP(fclose)(FILE *stream)
{
	...
	if(client.interpret_stream(stream))
	{
		_LOG("CBB fclose\n");
		client.flush_stream(stream);
		return client.close_stream(stream);
	}
	else
	{
		MAP_BACK(fclose);
...
extern "C" int CBB_WRAP(fflush)(FILE *stream)
{
	...
	if(client.interpret_stream(stream))
	{
		...
		_LOG("CBB fflush\n");
		return client.flush_stream(stream);
	}
	else
	{
		MAP_BACK(fflush);
...
extern "C" void CBB_WRAP(setbuf)(FILE * stream, char * buf)
{
	...
	{
		_LOG("CBB setbuf\n");
		client.freebuf_stream(stream);
		client.setbuf_stream(stream, buf);
	}
	else
	{
		MAP_BACK(setbuf);
...
extern "C" int CBB_WRAP(fgetc)(FILE *stream)
{
	...
	{	
		//_LOG("CBB fgetc\n");
		size_t ret=client.read_stream(stream, &c, sizeof(char));
		if(sizeof(char) != ret)
		{
			return EOF;
		}
...
extern "C" char *CBB_WRAP(fgets)(char* s, int n, FILE* stream)
{
	...
	if(client.interpret_stream(stream))
	{
		...
		while(--n)
		{
			client.read_stream(stream, s, sizeof(char));
			if('\n' == *s)
			{
				*s='\0';
				return start;
...
extern "C" int CBB_WRAP(fputc)(int c, FILE* stream)
{
	...
	{
		//_LOG("CBB fputc\n");
		size_t ret=client.write_stream(stream, &c, sizeof(char));
		if(sizeof(char) != ret)
		{
			return EOF;
		}
...
extern "C" int CBB_WRAP(fputs)(const char* s, FILE *stream)
{
	...
	if(client.interpret_stream(stream))
	{
		...
		int len=strlen(s);
		_LOG("string=%s\n", s);
		client.write_stream(stream, s, len*sizeof(char));
		return 1;
	}
	else
	{
...
extern "C" size_t CBB_WRAP(fread)(void *ptr, size_t size, size_t nitems, FILE *stream)
{
	...
	if(client.interpret_stream(stream))
	{
		...
		return client.read_stream(stream, ptr, IO_size);
	}
	else
	{
		MAP_BACK(fread);
...
extern "C" size_t CBB_WRAP(fwrite)(const void *ptr, size_t size, size_t nitems, FILE *stream)
{
	...
	if(client.interpret_stream(stream))
	{
		...
		_LOG("file no=%d\n", fileno(stream));
		size_t IO_size=size*nitems;
		return client.write_stream(stream, ptr, IO_size);
	}
	else
	{
		MAP_BACK(fwrite);
...
extern "C" int CBB_WRAP(fseek)(FILE *stream, long offset, int whence)
{
	...
	if(client.interpret_stream(stream))
	{
		_LOG("CBB fseek\n");
		_LOG("offset %ld\n", offset);
		return client.seek_stream(stream, offset, whence);
	}
	else
	{
		MAP_BACK(fseek);
...
extern "C" long CBB_WRAP(ftell)(FILE *stream)
{
	...
	{
		_LOG("CBB ftell\n");
		long pos=client.tell_stream(stream);
		_LOG("CBB pos=%ld\n", pos);
		return pos;
	}
	else
...
extern "C" void CBB_WRAP(rewind)(FILE *stream)
{
	...
	{
		_LOG("CBB rewind\n");
		client.seek_stream(stream, 0, SEEK_SET);
	}
	else
	{
		MAP_BACK(rewind);
...
extern "C" int CBB_WRAP(feof)(FILE *stream)throw()
{
	...
	{
		_LOG("CBB feof\n");
		return client.eof_stream(stream);
	}
	else
	{
		MAP_BACK(feof);
...
extern "C" int CBB_WRAP(ferror)(FILE *stream)throw()
{
	...
	{
		_LOG("CBB ferror\n");
		return client.error_stream(stream);
	}
	else
	{
		MAP_BACK(ferror);
...
extern "C" off_t CBB_WRAP(ftello)(FILE *stream)
{
	...
	{
		_LOG("CBB ftello\n");
		return client.tell_stream(stream);
	}
	else
	{
		MAP_BACK(ftello);
...
extern "C" int CBB_WRAP(fileno)(FILE *stream)throw()
{
	...
	{
		_LOG("CBB fileno\n");
		int fd=client.fileno_stream(stream);
		_LOG("fd=%d\n", fd);
		return fd;
	}
	else
