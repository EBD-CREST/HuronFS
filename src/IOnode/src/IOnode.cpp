...
{
	CBB_data_sync::
	set_queues(&_communication_input_queue.at(DATA_SYNC_QUEUE_NUM),
...
{ 
	...
	extended_IO_task* response=
		get_communication_input_queue(SERVER_THREAD_NUM)->get_task();
	ssize_t id=-1;
	response->pop(id);
		       ...
		       const char* 	node_ip)
{
	...
	extended_IO_task* response=
		get_communication_input_queue(0)->get_task();
	response->pop(ret);
